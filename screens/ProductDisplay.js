import { Image, StyleSheet, Text, View, Pressable, Dimensions, ScrollView, RefreshControl, TextInput } from "react-native";
    ...
    <View>
        ...
        <View style= {{marginBottom: 120}}>
            ...
            <View  style={{paddingHorizontal: '5%', marginVertical: '4%'}} >
                ...
                <View style = {{gap: 15, marginVertical: 30}}>
                    ...
                    <IncrementDecrementBtn minValue={quantity} onIncrease={()=>{if(plus.length >= 2 || !route.params.extras){handleAddToCartInc(route.params)}}} onDecrease={()=>handleRemoveFromCart(route.params)}/>
                    </View>
              ...
              style={{ lineHeight: 31, color: "#aaa",
              fontSize: 15, }}>{route.params.description ?? `Rainbow NERDS surround fruity, gummy centers. Those sweet little sparks are fantastic inventors. A poppable cluster, packed with tangy, crunchy NERDS. A candy so tasty, there aren’t even words.
              NUTRITIONAL INFO` }</Text>

              

              {lengthMore && <Text onPress={toggleNumberOfLines} style={{ lineHeight: 21, marginVertical: 10 , textAlign: 'right', color: '#BC6C25'}}>{textShown ? 'Read less...' : 'Read more...'}</Text>}
              {route.params.addOn && <View style={{gap: 25, paddingTop: 30, marginBottom: 50}}>
                            ...
                            {route.params.extras.map((item, idx)=><View key={idx} style={{flexDirection: 'row', justifyContent: 'space-between', borderBottomWidth : 1, borderColor: 'rgba(0,0,0,0.05)', paddingBottom: 15}}>
                                <View>
              ...
              {route.params.options &&<View><View style={{flexDirection: 'row', justifyContent: 'space-between', borderBottomWidth : 1, borderColor: 'rgba(0,0,0,0.05)', paddingBottom: 15,}}>
                                <Text style={{color: "black",fontWeight: "900",fontSize: 19,}}>{'Choose One'}</Text>
                                {/* <View style={{padding: 6, borderRadius: 15, backgroundColor:'rgba(0,0,0,0.1)'}}><Text  style={{color: "black",fontWeight: "bold",fontSize: 13,}}>Required</Text></View> */}
                            </View>
              {route.params.options.map((item, idx)=><View key={idx} style={{flexDirection: 'row', justifyContent: 'space-between', borderBottomWidth : 1, borderColor: 'rgba(0,0,0,0.05)', paddingVertical: 13,}}>
                                <View>
                                    <Text  style={{color: "black",fontWeight: "900",fontSize: 16,}}>{item}</Text>
                                </View>
              ...
              {route.params.nutrient && route.params.nutrient == 'protein' && <View style={{gap: 25, paddingTop: 30}}>
                            <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
                            ...
                            {route.params.extras.map((item, idx)=><View key={idx} style={{flexDirection: 'row', justifyContent: 'space-between', borderBottomWidth : 1, borderColor: 'rgba(0,0,0,0.05)', paddingBottom: 15}}>
                                ...
                                <View>
                                {(plus.length < 2 || (plus.length && plus.indexOf(item[0]) !== -1)) && <IncrementDecrementBtn minValue={foodDictionary[item[0]]} onIncrease={()=>{if (plus.length < 2){setFoodDictionary((prev)=>{return {...prev, [item[0]] : foodDictionary[item[0]]+1}});  handleAddToCart({'title': item[0], 'oldPrice': item[1], quantity: foodDictionary[item[0]]}); console.log(item[0]);setPlus((prev)=> {const arr = [...prev]; arr.push(item[0]); return arr})}}}  onDecrease={()=>{setFoodDictionary((prev)=>{return {...prev, [item[0]] : (foodDictionary[item[0]] ?foodDictionary[item[0]] : 1) -1}}); handleRemoveFromCart({'title': item[0], 'oldPrice': item[1], quantity: foodDictionary[item[0]]}); setPlus((prev)=>{const arr = [...prev]; arr.splice(prev.indexOf(item[0]), 1); return arr})}}/>}
                                </View>
                            </View>)}

                        </View>}
                    {route.params.instructions && <View 
      ...
      /></View>
                    ...
                    <View style={styles.catHead}>
                        ...
                        <ProductCategory items={categoryObject[route.params.category].slice(0,7).filter(item => item.title !== route.params.title)} onPress={handleAddToCart}/>
                    </View>
                    <View style={styles.catHead}>
        ...
        <View style={{flex: 1, width: "100%", minHeight: '60%', flexWrap: 'wrap', paddingVertical: '7%', position: "absolute",bottom: 0, zIndex: 1, backgroundColor: 'white' , flexDirection: 'row', justifyContent: "space-around", alignItems: 'center'}}>
            ...
            <View style ={{width: '40%', height: 70}}>
                <FlexButton onPress={()=>{if(plus.length >= 2 || !route.params.extras)handleAddToCart(route.params)}} background={'#283618'}><FontAwesome name="shopping-bag" size={24} color="white" /><Text style={{color: 'white'}}>Add to cart</Text></FlexButton>
            </View>
        </View>
        {/* {visible && <View style ={{
